...
class UserController extends Controller {
	...
	function add(){
 		...
 		$role=D('role')->select();
 		if (!empty($_POST)){
 			if($_POST['manager_pw'] != $_POST['manager_pw2']){
 				$ts="<font color='red'>两次密码不一致！</font>";
 			}else{
 				$_POST['manager_atime'] =time();
 				$_POST['manager_pw'] = md5($_POST['manager_pw']);
	...
	function show(){
		...
		$rinfo=array();
		foreach ($join as $k=>$v){
			$rinfo[$v['role_id']] = $v['role_name'];   //将查询的二维数组转换成一维对应的数组如：array(1 =>经理 ，2=>主管)
		}
	...
	//		var_dump($_POST);
			$_POST['manager_atime']=time();
			$save=$manager->save($_POST);
			if ($save){
				$this->success('设置成功',U('show'));
			}else {
				$this->error('设置失败！',U('show'));
			}
		}
    ...
    function login() {
    	...
    	if (!empty($_POST)){
    		if ($verify->check($_POST['verify'])){     //利用tp框架系统方法验证验证码。
    			$user=D('Manager');                    //实例化模型验证方法
    			$rst=$user->checkNamePw($_POST['manager_name'],md5($_POST['manager_pw']));  //调用模型内验证方法
    			if ($rst===false){
    				$cw= '用户名或密码错误！';
    				...
    				//认证成功后设置session;
//    
//     				session('manager_tname',$rst['manager_tname']);
//     				session('manager_id',$rst['manager_id']);
    ...
    /*//验证码调用方法
     * 
     * ***由于继承了公共控制器,验证码不显示，所以将验证码单独做了一个类为VerifyController.class.php,进行调用
    function verifyImg(){
    	$config=array(
