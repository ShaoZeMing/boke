...
class IndexController extends Controller{
    function index(){
        $where="blog_type=0 and class_id !=12";                                                //查询条件
...
//关于我
    ...
    //碎言碎语
   function moodlist($id= null){
       $where="blog_type=0 and class_lid=$id";                              //查询条件
       $page=$this->page($where);                                           //调用page方法得到分页对象
     //  $info=$this->select($where,$page->limit());
       $blog = D('Blog');
       $sql="select * from  bk_blog
             ...
             where {$where} ORDER BY blog_time DESC {$page->limit()}";
       $info = $blog->query($sql);
       //遍历数据，并将数据博文反转译输出
       foreach($info as $k => $v){
           $str = htmlspecialchars_decode($info[$k]["blog_content"]);   //反转译
           $info[$k]["blog_content"] = strip_tags($str);
       }
       $linfo=$this->lanmu_list();                                          //查询栏目
       $p = $page->show(4);                                                 //分页页码
       $this->assign('info', $info);
...
//留言板
    ...
    //子分类排列
    function class_lists($id=null){
        $where="blog_type=0 and class_id=$id";                             //查询条件
        $page=$this->page($where);                                          //调用page方法得到分页对象
        $info=$this->select($where,$page->limit());                         //调用多表查询方法查询
        $p = $page->show(4);                                                //分页页码
        $linfo=$this->lanmu_list();                                         //查询栏目
        $now_list=$this->now_list($where);                                  //查询最新数据排行
        $click_list=$this->click_list($where);                              //查询点击访问量排行
        $lid=M('Class')->find($id);                                          //查询子分类的父级id
        $class=$this->class_list($lid['class_lid']);                         //利用方法查询对应分类列表
        $this->assign('class',$class);
        $this->assign('info', $info);
        $this->assign('now', $now_list);
        $this->assign('click', $click_list);
    ...
    function read($id=null){
        if(!empty($id)){
            ...
            $b->save($click);
            $linfo=$this->lanmu_list();
            $where="blog_type=0 and class_id={$blog['class_id']}";                             //查询条件
            $now_list=$this->now_list($where);
            $click_list=$this->click_list($where);
    ...
    function now_list($where){
        $now_list=D('Blog')->join('bk_class ON bk_blog.blog_cid = bk_class.class_id')
            ->where($where)->order('blog_time DESC')->limit("0,9")->select();
        return $now_list;
 ...
 //最热文章排序
    function click_list($where){
        $click_list=D('Blog')->join('bk_class ON bk_blog.blog_cid = bk_class.class_id')
            ->where($where)->order('blog_click DESC')->limit("0,5")->select();
        return $click_list;
    }
 //栏目排序查询
    function lanmu_list(){
...
//查询数据方法
    function select($where,$limit){
        ...
        foreach($info as $k => $v){
            $str = htmlspecialchars_decode($info[$k]["blog_content"]);   //反转译
            //利用正则表达式替换<img>标签   //暂时不用
/*            $pattern = "/<[img|IMG].*?src=[\'|\"](.*?(?:[\.gif|\.jpg]))[\'|\"].*?[\/]?>/";
//            $strr=preg_replace($pattern,'',$str);*/
            $strr=strip_tags ($str);      //去除一切htnl标签元素。
            $info[$k]["blog_content"] = mb_strcut($strr,0,420) ."......";   //截取字符串
        }
        return $info;
    }
